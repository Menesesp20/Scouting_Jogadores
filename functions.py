...
# Function to calculate distance to opponent's goal
def distance_to_goal(x, y):
    goal_x = 100
    goal_y = 50
    return np.sqrt((goal_x - x)**2 + (goal_y - y)**2)

def progressive_Pass(df):
    # Filter the dataframe to get rows where the event is a "Pass"
    pass_df = df[df['typedisplayName'] == "Pass"].copy()

    # Calculate the distance to opponent's goal before and after the pass
    pass_df['start_distance_to_goal'] = distance_to_goal(pass_df['x'], pass_df['y'])
    pass_df['end_distance_to_goal'] = distance_to_goal(pass_df['endX'], pass_df['endY'])

    # Check if the pass was progressive
    pass_df['isProgressive'] = (pass_df['end_distance_to_goal'] < 0.75 * pass_df['start_distance_to_goal'])

    # Merge the isProgressive column to the original dataframe
    df = df.merge(pass_df[['id', 'isProgressive']], on='id', how='left')
    df['isProgressive'] = df['isProgressive'].fillna(False)

    return df

def create_Carry(df):
    # Approach:
    # 1. We'll traverse through the dataframe row by row.
    # 2. We'll keep track of the starting player and their team.
    # 3. If we encounter an event from the other team that isn't one of the defensive actions, 
    #    or if the ball goes to another player on the same team and doesn't return to the starting player, the sequence ends.
    # 4. If the sequence has 3 or more events, we'll mark it as a carry.

    # List of defensive actions by opponents
    defensive_actions = ["Tackle", "Aerial", "Interception", "Challenge"]
    ...
    for idx, row in df.iterrows():
        ...
        if not current_sequence:
            starting_player = row['name']
            starting_team = row['team']
            current_sequence.append(idx)
            continue
        
        # If the event is from the other team and isn't one of the defensive actions, end the sequence
        if row['team'] != starting_team and row['typedisplayName'] not in defensive_actions:
            if len(current_sequence) >= 3:
                carry_sequences.extend(current_sequence)
            current_sequence = []
            continue
        
        # If the ball goes to another player and doesn't return to the starting player, end the sequence
        if row['name'] != starting_player and row['Receiver'] != starting_player:
            if len(current_sequence) >= 3:
                carry_sequences.extend(current_sequence)
            current_sequence = []
            continue
    ...
    # Update the isCarry column based on the identified sequences
    df['isCarry'] = df.index.isin(carry_sequences)

    # Initialize the new columns with NaN values
    df['carry_X'] = np.nan
    df['carry_Y'] = np.nan
    df['carry_endX'] = np.nan
    df['carry_endY'] = np.nan

    # Identify the start and end of each carry sequence
    start_of_carry = df['isCarry'] & ~df['isCarry'].shift(fill_value=False)
    end_of_carry = df['isCarry'] & ~df['isCarry'].shift(-1, fill_value=False)

    # Set the initial location for each start of a carry sequence
    df.loc[start_of_carry, 'carry_X'] = df.loc[start_of_carry, 'x']
    df.loc[start_of_carry, 'carry_Y'] = df.loc[start_of_carry, 'y']

    # Propagate the initial location forward for each subsequent row in the carry sequence
    df['carry_X'] = df['carry_X'].fillna(method='ffill')
    df['carry_Y'] = df['carry_Y'].fillna(method='ffill')

    # Set the ending location for each row in the carry sequence
    df.loc[df['isCarry'], 'carry_endX'] = df.loc[df['isCarry'], 'x']
    df.loc[df['isCarry'], 'carry_endY'] = df.loc[df['isCarry'], 'y']

    return df

def get_values_by_qualifier(qualifiers, key, values):
    qualifiers = json.loads(qualifiers.replace("'", "\""))
    result = []
    for q in qualifiers:
        if isinstance(q, dict):
            for k, v in q.items():
                if isinstance(v, dict):
                    if v.get('value') in values:
                        result.append(v.get('displayName'))
                elif k == key and v in values:
                    result.append(v)
...
def xT_Intercepted(df):
    # Define defensive actions
    defensive_actions = ['Ball Recovery', 'Aerial', 'Tackle', 'Clearance', 'Interception', 'Challenge']

    # Create a new column 'xT Intercepted' with 0 values
    df['xT Intercepted'] = 0

    # Get unique game_ids
    game_ids = df['Match_ID'].unique()

    # Initialize an empty list to store the individual game DataFrames
    games = []

    for game_id in game_ids:
        # Subset df by game_id and sort by timestamp
        df[df['Match_ID'] == game_id].sort_values(by=['Match_ID', 'matchTimestamp'], inplace=True, ascending=[True, True])

        # Iterate over the game dataframe by index
        for i in range(len(df) - 1):  # '-1' because we will look ahead to the 'next' row
            # Check if current action is 'Pass' and next action is a defensive one
            if df.loc[i, 'typedisplayName'] == 'Pass' and df.loc[i+1, 'typedisplayName'] in defensive_actions:
                # Check if teams are different
                if df.loc[i, 'team'] != df.loc[i+1, 'team']:
                    # Assign xT value to 'xT Intercepted' for the defensive action
                    df.loc[i+1, 'xT Intercepted'] = df.loc[i, 'xT']
        
        # Add the game df to the list
        games.append(df)
    
    # Concatenate all the game DataFrames to create the new df
    df_new = pd.concat(games, ignore_index=True)
...
def metric_1v1(df):
    ...
    # Create a new column '1v1' with False values
    df['1v1'] = False
    ...
    for game_id in game_ids:
        ...
        # Iterate over the game dataframe by index
        for i in df.index[:-1]:  # '-1' because we will look ahead to the 'next' row
            # Check if current action is 'Take On' and next action is a defensive one
            if df.loc[i, 'typedisplayName'] == 'Take On' and df.loc[i+1, 'typedisplayName'] in defensive_actions:
                # Check if teams are different
                if df.loc[i, 'team'] != df.loc[i+1, 'team']:
                    # Assign True to '1v1' for the defensive action
                    df.loc[i+1, '1v1'] = True
...
def create_CarryProg(df):
    # Group the indices in carry_sequences into sequences
    carry_sequence_groups = []
    current_sequence = []
    carry_sequences = []
    
    for idx in carry_sequences:
        if current_sequence and idx != current_sequence[-1] + 1:
            carry_sequence_groups.append(current_sequence)
            current_sequence = []
        current_sequence.append(idx)
    ...
    # For each carry sequence group, determine if it's a progressive carry
    progressive_carry_indices = []

    for sequence in carry_sequence_groups:
        start_event = df.iloc[sequence[0]]
        end_event = df.iloc[sequence[-1]]
        
        start_distance_to_goal = 100 - start_event['carry_X']
        end_distance_to_goal = 100 - end_event['carry_endX']
        
        # Check if the end distance is at least 15% less than the start distance
        if end_distance_to_goal <= 0.85 * start_distance_to_goal:
            progressive_carry_indices.extend(sequence)

    # Update the isCarryProg column based on the identified progressive carry indices
    df['isCarryProg'] = df.index.isin(progressive_carry_indices)
...
def cluster_Event(df, teamName, event_name, n_clusters):
    cols_Cluster = ['team', 'typedisplayName', 'qualifiers', 'x', 'y', 'endX', 'endY', 'Receiver', 'Length']
    ...
    if df_cluster.empty:
        return pd.DataFrame(columns=['team', 'typedisplayName', 'qualifiers', 'x', 'y', 'endX', 'endY', 'cluster', 'Receiver', 'Length'])
...
def highTurnovers(df, club, player=None):

    colors = ['#E63838', '#3A75C4', '#31ca27', '#181818']
    ...
    homeTeam = df.home_Team.unique()[0]
    league = df['League'].unique()[0]

    awayTeam = df.away_Team.unique()[0]
    ...
    #Plotting the pitch
    highTurnover = df.loc[(df['High_Recovery'] == True) & (df.team == club)].reset_index(drop=True)
    highTurnover.drop_duplicates(['name', 'typedisplayName', 'High_Recovery', 'x', 'y'], keep='first')
    
    #dfShot = shotAfterRecover(df, club)
    #dfShot = dfShot.loc[(dfShot.x >= 65) & (dfShot.xG >= 0)].reset_index(drop=True)

    fig, ax = plt.subplots(figsize=(10, 8), dpi=300)

    pitch = VerticalPitch(pitch_type='opta',
                            pitch_color=pitchColor, line_color=lineColor, half = True,
                            line_zorder=1, linewidth=1, spot_scale=0.00)
    ...
    fig.suptitle(club, fontsize=35, color=lineColor, fontproperties=custom_font, y=0.95)

    fig_text(s = 'High Turnovers | Made by: @menesesp20',
             fontproperties=custom_font,
             x = 0.5, y = 0.89, color=lineColor, ha='center', fontsize=8);
    ...
    ax.axhline(65,c=color, ls='--', lw=4)

    ax.scatter(highTurnover.y, highTurnover.x, label = 'High Turnovers' + ' ' + '(' + f'{len(highTurnover)}' + ')',
                        c=colorHeat[0], marker='o', edgecolor=lineColor, s=150, zorder=3)

    #ax.scatter(dfShot.y, dfShot.x, label = 'Shot after a turnover within 5 seconds' + ' ' + '(' + f'{len(dfShot)}' + ')',
    #                    c='#ffba08', marker='*', edgecolor='black', s=500, zorder=3)

    #Criação da legenda
    l = ax.legend(bbox_to_anchor=(0.04, 0.27), loc='upper left', facecolor=lineColor, framealpha=0, labelspacing=.8)
    ...
    # Club Logo
    fig = add_image(image=f'./Images/Clubs/{league}/{club}.png', fig=fig, left=0.14, bottom=0.88, width=0.2, height=0.09)

    if player != None:
        # Player Image
        fig = add_image(image='Images/Players/' + league + '/' + club + '/' + 'Florentino Luís' + '.png', fig=fig, left=0.68, bottom=0.91, width=0.2, height=0.11)
...
def buildUpPasses(df, club, Seconds, length):
    from datetime import timedelta
    ...
    for idx, row in teamDF.iterrows():
        if 'GoalKick' in row['qualifiers']:
            if  row['Length'] <= length:
                tempo = row['matchTimestamp']
                jogadas = teamDF.loc[(teamDF['matchTimestamp'] > tempo) & (teamDF['matchTimestamp'] <= tempo + timedelta(seconds=Seconds))]
            
                for i in jogadas.index.unique():
                    if teamDF.iloc[i]['typedisplayName'] == 'Pass':
                        if contador == 0:
                            contador = 1
                            eventsGK = pd.DataFrame([row[cols].values], columns=cols)
                            passesBuildUp = pd.concat([passesBuildUp, eventsGK], ignore_index=True)

                        eventsGK = pd.DataFrame([teamDF.loc[i, cols].values], columns=cols)
                        passesBuildUp = pd.concat([passesBuildUp, eventsGK], ignore_index=True)
...
def draw_heatmap_construcao(df, club, Seconds, length):
        ...
        passesGk = buildUpPasses(df, club, Seconds, length)
        ...
        pearl_earring_cmap = LinearSegmentedColormap.from_list("Pearl Earring - 10 colors",
                                                                [pitchColor, colorHeat[0]], N=20)
        passesGk['x'] = passesGk['x'].astype(float)
        passesGk['y'] = passesGk['y'].astype(float)

        bs = pitch.bin_statistic(passesGk['x'], passesGk['y'], bins=(21, 17))

        pitch.heatmap(bs, edgecolors=pitchColor, ax=ax, cmap=pearl_earring_cmap)

        fig.suptitle('How do they come out playing?', fontproperties=custom_font, fontsize=35, color=lineColor,
                        fontweight = "bold", x=0.53, y=1.07)

        fig_text(s = f"{Seconds} seconds after GoalKick | Made by: @menesesp20",
                x = 0.5, y = 1,
                fontproperties=custom_font,
                color=lineColor, fontweight='bold', ha='center', fontsize=16);
        
        # Club Logo
        fig = add_image(image=f'./Images/Clubs/{league}/{club}.png', fig=fig, left=0.1, bottom=0.98, width=0.2, height=0.1)
        ...
        ax.annotate('', xy=(0.3, -0.07), xycoords='axes fraction', xytext=(0.7, -0.07), 
                arrowprops=dict(arrowstyle="<-", color=lineColor, lw=2))

def GoalKick(df, club, n_cluster, length):
        ...
        #goalKick = goalKick[(goalKick.Length <= length)].reset_index(drop=True)
        ...
        pitch = Pitch(pitch_type='opta', pitch_color=pitchColor,
                        line_color=lineColor, line_zorder=1,
        ...
        fig.suptitle('How do they come out playing?', fontsize=35, color=lineColor,
                fontweight = "bold", x=0.53, y=0.95)

        fig_text(s = "GoalKick | Made by: @menesesp20",
                x = 0.5, y = 0.9,
                color=lineColor, fontweight='bold', ha='center' ,fontsize=16);
        ...
        print(goalKick)
        cluster1st = goalKick[(goalKick.cluster == 1)]['Receiver'].value_counts()
        cluster2nd = goalKick[(goalKick.cluster == 0)]['Receiver'].value_counts()
        cluster3rd = goalKick[(goalKick.cluster == 2)]['Receiver'].value_counts()

        fig_text(s = f'Most frequent zone: {cluster1st.idxmax()}\n(Target player)',
                x = 0.78, y = 0.79,
                color='#ea04dc', fontweight='bold', ha='center' ,fontsize=10);

        fig_text(s = f'Second most frequent zone: {cluster2nd.idxmax()}\n(Target player)',
                x = 0.78, y = 0.75,
                color='#2d92df', fontweight='bold', ha='center' ,fontsize=10);

        fig_text(s = f'Third most frequent zone: {cluster3rd.idxmax()}\n(Target player)',
                x = 0.78, y = 0.71,
                color='#fb8c04', fontweight='bold', ha='center' ,fontsize=10);

        # Club Logo
        fig = add_image(image=f'./Images/Clubs/{league}/{club}.png', fig=fig, left=0.1, bottom=0.865, width=0.2, height=0.1)
...
def player_Network(df, playerName, team, gameDay, afterSub=None, axis=False):
        ...
        pearl_earring_cmap = LinearSegmentedColormap.from_list("Pearl Earring - 10 colors",
                                                                [pitchColor, colorHeat[0]], N=10)

        bs = pitch.bin_statistic(passes_Player['endX'], passes_Player['endY'], bins=(6, 3))

        pitch.heatmap(bs, edgecolors=pitchColor, ax=ax, cmap=pearl_earring_cmap, alpha=0.5)
...
######################################################################################################################################################
        ...
        for index, row in avg.iterrows():
                pitch.annotate(row.passer, xy=(row['x_avg'], row['y_avg']), c=lineColor, va='center', ha='center', fontproperties=custom_font, size=sizeName, ax=ax, bbox=dict(facecolor='#181818', alpha=0.7, edgecolor='none'))
        ...
        if axis == False:
                ...
                # Club Logo
                fig = add_image(image=f'./Images/Clubs/{league}/{team}.png', fig=fig, left=0.33, bottom=0.905, width=0.06, height=0.05)
        else:
                pass
...
def passing_networkWhoScored(df, team, gameDay, afterSub=None, axis=False):
        ...
        network['recipient'] = network['passer'].shift(-1)
        # Set the last row's 'recipient' to the 'name' value of the last row
        network.at[network.index[-1], 'recipient'] = network.at[network.index[-1], 'name']
        ...
        for pair_key, row in pair_stats.iterrows():
            ...
            norm = Normalize(vmin=0, vmax=max_pair_value)
            edge_cmap = LinearSegmentedColormap.from_list("Pearl Earring - 10 colors",
                                                                ['#e8e8e8', colorHeat[0]], N=10)
            edge_color = edge_cmap(norm(pass_value))

            ax.plot([player1_y - 2, player2_y - 2], [player1_x - 2, player2_x - 2],
                    '<--', linestyle='-', alpha=1, lw=num_passes, zorder=2, color=edge_color)

            ax.plot([player2_y - 2, player1_y - 2], [player2_x - 2, player1_x - 2],
                    '<--', linestyle='-', alpha=1, lw=num_passes, zorder=2, color=edge_color)
...
######################################################################################################################################################
        ...
        if axis == False:
                ...
                fig_text(s = f'<{homeTeam}> vs <{awayTeam}>',
                        x = 0.5, y = 0.915,
                        color=lineColor, fontproperties=custom_font, ha='center',
                        highlight_textprops = highlight_textprops,
                        fontsize=30);
                ...
                # Club Logo
                #fig = add_image(image='Images/Clubs/{league}/{team}.png' + league + '/' + team + '.png', fig=fig, left=0.35, bottom=0.905, width=0.06, height=0.05)
        else:
                                #Params for the text inside the <> this is a function to highlight text
                highlight_textprops =\
                        [{"color": color[0],"fontweight": 'bold'},
                ...
                ax_text(s = f'<{homeName}>' + ' ' + 'vs' + ' ' + f'<{awayName}>',
                        x = 52, y = 108,
                        color=lineColor, fontproperties=custom_font, ha='center',
                        highlight_textprops = highlight_textprops,
                        fontsize=11, ax=axis);
        ...
        # Create a diagonal marker
        verts = np.array([[0, 0], [1, 1]])
        codes = [Path.MOVETO, Path.LINETO]
        diagonal_marker = Path(verts, codes)

        legende_linesColor = []

        for i in range(num_elements):
                line = mlines.Line2D([], [], color=colorHeat[0], marker='o', markersize=circle_size, alpha=(i + 1) / num_elements, linestyle='None')
                circle_size += 1.5
                legend_entries.append(line)

                # For lines with increasing width
                line_width = mlines.Line2D([], [], color=lineColor, marker=diagonal_marker, linestyle='None',
                                           markersize=25, markeredgewidth=linewidth)
                linewidth += 0.7
                legend_entries_lines.append(line_width)

                # For lines with increasing value
                if i == 0:
                        color = '#181818'
                        alpha = 1
                elif i == 1:
                ...
                line_width_xT = mlines.Line2D([], [], color=color, alpha=alpha, marker=diagonal_marker, linestyle='None',
                                                markersize=25, markeredgewidth=linewidth)
                linewidth += 0.7
                legende_linesColor.append(line_width_xT)

        legend_labels = [""] * num_elements
        
        legend_circles  = ax.legend(handles=legend_entries, labels=legend_labels, loc="lower center", ncol=num_elements,
                                    handletextpad=0, columnspacing=1, handlelength=1, frameon=False, bbox_to_anchor=(0.5, -0.02))
        ...
        legend_lines = ax.legend(handles=legend_entries_lines, labels=legend_labels, loc="lower center", 
                                ncol=num_elements, handletextpad=0, columnspacing=0.8, handlelength=1, 
                                bbox_to_anchor=(0.25, -0.025), frameon=False)

        fig_text(s = 'Pass link count',
                                x = 0.417, y = 0.12,
                                color=lineColor, fontweight='bold',
                                fontproperties=custom_font,
                                ha='center', va='center',
        ...
        legend_linesColors = ax.legend(handles=legende_linesColor, labels=legend_labels, loc="lower center",
                                 ncol=num_elements, handletextpad=0, columnspacing=0.8, handlelength=1,
                                 bbox_to_anchor=(0.75, -0.025), frameon=False)

        fig_text(s = 'Pass link value',
                                x = 0.62, y = 0.12,
                                color=lineColor, fontweight='bold',
                                fontproperties=custom_font,
                                ha='center', va='center',
...
def heatMap_xT(df, club, player=None):
        ...
        if (player == None):
                xTheatMap = dfXT.loc[(dfXT['team'] == club)].reset_index(drop=True)
        else:
                xTheatMap = dfXT.loc[(dfXT['name'] == player)].reset_index(drop=True)

        homeTeam = xTheatMap.home_Team.unique()[0]
        league = xTheatMap.League.unique()[0]
        gameDay = xTheatMap['Match_ID'].unique()[0]

        awayTeam = xTheatMap.away_Team.unique()[0]
        ...
        bs = pitch.bin_statistic(xTheatHeat['x'], xTheatHeat['y'], bins=(21, 17))
        pitch.heatmap(bs, edgecolors=pitchColor, ax=ax, cmap=pearl_earring_cmap)

        # Club Logo
        fig = add_image(image=f'./Images/Clubs/{league}/{club}.png', fig=fig, left=-0.01, bottom=1, width=0.2, height=0.12)

        # TITLE
        if player == None:
                fig_text(s = 'Where' + ' ' + club + ' ' + 'generate the most xT',
                         fontproperties=custom_font,
                        x = 0.54, y = 1.1, color=lineColor, fontweight='bold', ha='center' ,fontsize=29.5);
        else:
                fig_text(s = 'Where' + ' ' + player + ' ' + 'generate the most xT',
                         fontproperties=custom_font,
                        x = 0.54, y = 1.1, color=lineColor, fontweight='bold', ha='center' ,fontsize=27);     
        ...
        fig_text(s = str(round(sum(xTheatMap.xT), 2)) + ' ' + 'xT Generated', 
                x = 0.51, y = 1.02, color=lineColor,
                fontproperties=custom_font, fontweight='bold',
...
def finalThird(df, club, n_cluster):
        ...
        final3rd_Cluster = cluster_Event(df, club, 'KeyPass', n_cluster)
        ...
        #################################################################################################################################################
                ...
                #Params for the text inside the <> this is a function to highlight text
                highlight_textprops =\
                        [{"color": color[2],"fontweight": 'bold'},
                        {"color": color2[2],"fontweight": 'bold'}
                        ]

                fig_text(s =f'<{homeName}>' + ' ' + 'vs' + ' ' + f'<{awayName}>',
                         x = 0.53, y = 0.98, ha='center', va='center',
                         highlight_textprops = highlight_textprops ,
                         color=lineColor, fontproperties=custom_font,
                         fontsize=35);
                
                fig_text(s =  f'{league}' + ' ' + '|' + ' '  + str(matchDay) + ' ' + '| Season 2022-23 | @menesesp20',
                         x = 0.51, y = 0.94,
                         color=lineColor, fontproperties=custom_font,
                         ha='center', va='center',
                         fontsize=8);
        ...
        #################################################################################################################################################
                ...
                # Title of our plot
                fig.suptitle(club + ' ' + 'Final Third',
                             fontsize=35, color=lineColor,
                             fontproperties=custom_font,
                             x=0.525, y=1)
        ...
        fig_text(s = str(rightfinal3rdTotal) + ' ' + '%',
                x = 0.75, y = 0.5,
                color=lineColor, fontproperties=custom_font, ha='center' ,fontsize=25);
        ...
        fig_text(s =str(round(xT_Sides.right_xT[0], 2)),
                x = 0.73, y = 0.37,
                color=lineColor,fontproperties=custom_font, ha='center' ,fontsize=18);
        ...
        if right > left > center:
                ...
                rectangleLeft = patches.Rectangle([67, 50], width=33, height=50, linewidth = 2, linestyle='-',
                                edgecolor=lineColor, facecolor='#ff0000', alpha=0.5, zorder=1 )

                # CENTER ZONE
                rectangleCenter = patches.Rectangle([33.1, 50], width=33.8, height=50, linewidth = 2, linestyle='-',
                                edgecolor=lineColor, facecolor='#ff0000', alpha=0.3, zorder=1 )

                # RIGHT ZONE
                rectangleRight = patches.Rectangle([0, 50], width=33, height=50, linewidth = 2, linestyle='-',
                                edgecolor=lineColor, facecolor='#ff0000', alpha=0.8, zorder=1)
        ...
        for x in range(len(final3rd_Cluster['cluster'])):
                ...
                elif final3rd_Cluster['cluster'][x] == 3:
                        pitch.lines(xstart=final3rd_Cluster['x'][x], ystart=final3rd_Cluster['y'][x],
                                xend=final3rd_Cluster['endX'][x], yend=final3rd_Cluster['endY'][x],
                                color='#2d92df',
                                ax=ax,
                ...
                elif final3rd_Cluster['cluster'][x] == 0:
                        pitch.lines(xstart=final3rd_Cluster['x'][x], ystart=final3rd_Cluster['y'][x],
                                xend=final3rd_Cluster['endX'][x], yend=final3rd_Cluster['endY'][x],
                                color='#fb8c04',
                                ax=ax,
        ...
        # Club Logo
        fig = add_image(image=f'./Images/Clubs/{league}/{club}.png', fig=fig, left=0.1, bottom=0.91, width=0.2, height=0.1)

        # END NOTE
        fig_text(s = 'The values inside the diamond are the xT value for each third',
                 x = 0.5, y = 0.125,
                 color=lineColor, fontproperties=custom_font, ha='center' ,fontsize=11);

        fig_text(s = 'xT values based on Karun Singhs model',
                 x = 0.765, y = 0.875,
                 color=lineColor, fontproperties=custom_font, ha='center' ,fontsize=8);
...
def field_Tilt(df, axis=False):
    ...
    if axis == False:
        ...
        # Club Logo
        fig = add_image(image='./Images/Clubs/' + league + '/' + home + '.png', fig=fig,
...
def dashboardPlayerOffensive(df, league, team, player):
    ...
    # Club Logo
    add_image(image='Images/Clubs/' + league + '/' + team + '.png', fig=fig, left=0.12, bottom=0.9, width=0.08, height=0.05)
...
def deepCompletion_Plot(df, team, radius, against=None, player=None):
    
    if against != None:
        deepcompletion = df[(df['deepCompletion'] == True) & (df['team'] != team) & (df['x'] != 99.5)].reset_index(drop=True)
    else:
         deepcompletion = df[(df['deepCompletion'] == True) & (df['team'] == team) & (df['x'] != 99.5)].reset_index(drop=True)

    colors = ['#E63838', '#3A75C4', '#31ca27', '#181818']

    league = deepcompletion['League'].unique()
    ...
    fig, ax = pitch.grid(nrows=1, ncols=1, figheight=35,
    ...
    matchDay = deepcompletion.Match_ID.unique()[0]
    game = deepcompletion.Match.unique()[0]

    fig_text(s = 'A <Completed pass> ends in the highlighted zone',
                     fontproperties=custom_font,
                     x = 0.5, y = 0.935, highlight_textprops = highlight_textprops,
                     fontweight='bold', ha='center', fontsize=65, color=lineColor);

    fig_text(s = f'MatchDay: {str(matchDay)} {game} | @Menesesp20',
             fontproperties=custom_font,
             x = 0.5, y = 0.90,
             fontweight='bold', ha='center', fontsize=35, color=lineColor);
    ...
    fig_text(s = f'Nº DeepCompletation: <{len(deepcompletion)}> / Per game: <{round(len(deepcompletion) / 5)}>',
             highlight_textprops = highlight_textprops,
             fontproperties=custom_font,
             x = 0.5, y = 0.84,
             fontweight='bold', ha='center', fontsize=35, color=lineColor);
    ...
    circle = patches.Circle([50,100], radius = radius , linewidth = 7, linestyle='--',
                      edgecolor=lineColor, facecolor='none', alpha=0.9, zorder=3 )
    ...
    if player != None:
        df = deepcompletion[(deepcompletion['name'] == player)].reset_index(drop=True)

        ax['pitch'].set_title(player , fontsize=50, color=lineColor, fontweight = 'bold', va="center", ha="center")
    else:
        df = deepcompletion.copy()
    ...
    pitch.lines(df['x'], df['y'], df['endX'], df['endY'],
                ax=ax['pitch'], comet = True, color= color2, lw=10, alpha = 0.7)

    pitch.scatter(df['endX'], df['endY'], edgecolors= color2, c=pitchColor,
                  s = 500, zorder=4, ax=ax['pitch'], marker = 'o', alpha = 0.9,linewidths=5)

    # Club Logo
    fig = add_image(image=f'./Images/Clubs/{league}/{team}.png', fig=fig, left=0.11, bottom=0.89, width=0.08, height=0.08)

def possessionGained(df, league, team, eventType):
    ...
    pitch = Pitch(pitch_type='opta',
                pitch_color='#e8e8e8', line_color='#181818',
                line_zorder=3, linewidth=2, spot_scale=0.00)
    ...
    add_image(image=f'./Images/Clubs/{league}/{team}.png', fig=fig, left=0.25, bottom=0.953, width=0.08, height=0.09)

def plotPassesHeatMap(df, league, team, x=None):
    ...
    if x == None:
        ...
        # Club Logo
        fig = add_image(image=f'./Images/Clubs/{league}/{team}.png', fig=fig, left=0.26, bottom=0.9, width=0.08, height=0.09)
    ...
    fig_text(s = team + ' passes',
                x = 0.53, y = 0.97, fontweight='bold',
                ha='center', fontsize=35, fontproperties=custom_font, color=lineColor);

    passes = df.loc[(df['team'] == team) & (df['typedisplayName'] == 'Pass')].reset_index(drop=True)

    homeTeam = passes.home_Team.unique()[0]
    league = passes.League.unique()[0]

    awayTeam = passes.away_Team.unique()[0]
    ...
    path_eff = [path_effects.Stroke(linewidth=3, foreground=lineColor),
                path_effects.Normal()]

    bs = pitch.bin_statistic_positional(passes['x'], passes['y'],  statistic='count', positional='full', normalize=True)
        
    pitch.heatmap_positional(bs, edgecolors=pitchColor, ax=ax, cmap=pearl_earring_cmap, alpha=0.6)
    ...
    pitch.label_heatmap(bs, color=pitchColor, fontsize=fontsize,
                                ax=ax, ha='center', va='center',
                                fontproperties=custom_font,
                                str_format='{:.0%}', path_effects=path_eff)

def heatMapChances(df, team, gameID=None, player=None, x=None):
    ...
    if gameID != None:
        df = df[(df['Match_ID'] == gameID)].reset_index(drop=True)
    else:
        pass

    homeTeam = df.home_Team.unique()[0]
    league = df['League'].unique()
    game = df.Match.unique()[0]
    ...
    if x ==None:
        ...
        # Club Logo
        fig = add_image(image=f'./Images/Clubs/{league}/{team}.png', fig=fig, left=0.21, bottom=0.9, width=0.08, height=0.07)
    ...
    if player == None:
        fig_text(s = 'Where has ' + team + ' created from',
                    x = 0.5, y = 0.97, fontweight='bold',
                    fontproperties=custom_font,
                    ha='center', fontsize=16, color=lineColor);
        ...
        fig_text(s = 'Where has ' + player + ' created from',
                    ...
                    fontproperties=custom_font,
                    ha='center',fontsize=11, color=lineColor);

    fig_text(s = 'All Chances created',
                x = 0.50, y = 0.88, fontweight='bold',
                fontproperties=custom_font,
                ha='center', fontsize=8, color=lineColor, alpha=0.7);

    if x == None:
        nGames = len(df['Match_ID'].unique())
        fig_text(s = f'Last {nGames} games',
                    x = 0.5, y = 0.91, fontweight='bold',
                    ha='center', fontsize=8, fontproperties=custom_font, color=lineColor);
    else:
        ax_text(s = f'{game}',
                    x = 50, y = 111, fontweight='bold',
                    ha='center', fontsize=14, fontproperties=custom_font, color=lineColor, ax=x);
    ...
    pitch.label_heatmap(bs, color=lineColor, fontsize=fontsize,
                                ax=ax, ha='center', va='center',
                                fontproperties=custom_font,
                                str_format='{:.0%}', path_effects=path_eff)

def heatMap_xTIntercepted(df, club, player=None):
        ...
        dfXT = df.loc[(df['xT Intercepted'] >= 0)].reset_index(drop=True)

        if player == None:
                xTheatMap = dfXT.loc[(dfXT['team'] == club)]
        else:
                xTheatMap = dfXT.loc[(dfXT['name'] == player)]
        ...
        bs = pitch.bin_statistic(xTheatMap['x'], xTheatMap['y'], bins=(21, 17))
        pitch.heatmap(bs, edgecolors=pitchColor, ax=ax, cmap=pearl_earring_cmap)
        ...
        # TITLE
        if player == None:
                fig_text(s = f'Where the {club} avoids opportunities',
                         fontproperties=custom_font,
                        x = 0.54, y = 1.1, color=lineColor, fontweight='bold', ha='center' ,fontsize=29.5);
        else:
                fig_text(s = f'Where the {player} avoids opportunities',
                         fontproperties=custom_font,
                        x = 0.54, y = 1.1, color=lineColor, fontweight='bold', ha='center' ,fontsize=27);     
        ...
        # TOTAL xT Intercepted
        fig_text(s = str(round(sum(xTheatMap['xT Intercepted']), 2)) + ' ' + 'xT Intercepted Generated', 
                x = 0.51, y = 1.02, color=lineColor,
                fontproperties=custom_font, fontweight='bold',
                ha='center' ,fontsize=18);
